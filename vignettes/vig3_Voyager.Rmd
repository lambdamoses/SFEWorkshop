...
```{r, fig.width=8, fig.height=4, fig.alt="Plot of Visium spots in tissue in physical space, colored by number of myofibers intersecting each spot and proportion of area of each spot occupied by myofibers."}
plotSpatialFeature(sfe_tissue, features = c("n_myofibers", "prop_myofiber"), 
                   colGeometryName = "spotPoly")
```
...
Plotting proportion of mitochondrial counts vs. mean myofiber area, we also see two clusters, one with higher proportion of mitochondrial counts and smaller area, and another with lower proportion of mitochondrial counts and on average slightly larger area. Type IIa myofibers tend to have smaller area and larger proportion of mitochondrial counts.
...
sfe_tissue <- runCorrelogram(sfe_tissue, hvgs[1:5], colGraphName = "visium", 
                             order = 10, BPPARAM = MulticoreParam(2))
```

The results can be plotted with `plotCorrelogram`
```{r}
plotCorrelogram(sfe_tissue, hvgs[1:5])
```
...
Again, this can be done for `colData`, `annotGeometry`, and etc. as in Moran's I and permutation testing. 

# Limitations
1. The plotting functions don't plot the H&E image in the background. We may implement that if there is popular request.
2. It's more convoluted to trick `geom_sf` into flipping the y axis, since the coordinates are in pixels in full resolution image and the image has the origin at the top left.
3. In the first version of `Voyager`, only univariate global spatial autocorrelation metrics are supported. Anisotropy, univariate local spatial metrics, and multivariate spatial analyses will be added in later versions.
4. Again, only 2D data is supported at present.

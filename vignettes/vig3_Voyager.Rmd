...
```{r, fig.alt="Scatter plot with nCounts in the y axis and mean myofiber area in the x axis, where each point represents a Visium spot, colored by the number of myofibers intersecting the spot."}
...
```{r, fig.alt="Scatter plot with nGenes in the y axis and nCounts in the x axis. Each point represents a Visium spot in tissue, colored by mean myofiber area."}
plotColData(sfe_tissue, x = "nCounts", y = "nGenes", colour_by = "mean_myofiber_area")
```

```{r, fig.alt="Scatter plot with nGenes in the y axis and nCounts in the x axis. Each point represents a Visium spot in tissue, colored by number of myofibers."}
...
```{r}
plotCorrelogram(sfe_tissue, c("area", "eccentricity"),
                annotGeometryName = "myofiber_simplified")
```
...
For gene expression
```{r}
sfe_tissue <- runCorrelogram(sfe_tissue, hvgs[1:5], colGraphName = "visium", order = 10,
                             BPPARAM = MulticoreParam(2))
```
...
```{r}
set.seed(29)
corr_clusters <- clusterCorrelograms(sfe_tissue, hvgs[1:5], BLUSPARAM = KmeansParam(2))
head(corr_clusters)
```
...
```{r}
plotCorrelogram(sfe_tissue, features = hvgs[1:5], color_by = corr_clusters$cluster) +
  labs(color = "cluster")
```
